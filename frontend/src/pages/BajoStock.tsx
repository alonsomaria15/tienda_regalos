...
const BajoStock: React.FC = () => {
  ...
  // Función para asignar clase según el stock
  const getRowColor = (stock: number) => {
    if (stock <= 3) return "bg-red-200 text-red-800"; // muy bajo
    if (stock <= 5) return "bg-yellow-100 text-yellow-800"; // pronto será bajo
    ...
    <div className="p-6">
      <div className="bg-white shadow-md rounded-lg p-6 max-w-3xl">
        ...
        <table className="w-full border border-gray-300 mb-4">
          ...
          <tbody>
            {products.map((p) => (
              <tr key={p.id} className={`${getRowColor(p.stock)} border-b`}>
                ...
                <td className="border p-2">
                  {p.detalles ? (
                    <div className="flex flex-col">
                      {p.detalles.color && (
                        <span>Color: {p.detalles.color}</span>
                      )}
                      {p.detalles.talla && (
                        <span>Talla: {p.detalles.talla}</span>
                      )}
                      {p.detalles.modelo && (
